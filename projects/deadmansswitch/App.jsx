...
const BlogTextProps_Methodology = {
  id: "",
  text: "<p class=\\\"blogtextlist\\\">- The very first thing which enters the mind for the best way to handle such confidential information is to home host a server on my personal desktop. That simply won't work though because I power my PC off at night and when I'm not using it which is at MINIMUM 8 hours a day. I can't risk a third of my day not being covered that is insanity. So I was forced to host the server remotely. Costs would surely accrue with time so that was a concern. I researched ways to remotely host a node.js server 24/7 for free, and came across adaptable.io. You post your code to GitHub, hook an adaptable.io server into the repo with build/execution scripts, and boom. Easy free 24/7 server.</p><br><p class=\\\"blogtextlist\\\">- The next hurdle was security. The repo source code is viewable by all unless you make them private, which makes them hidden from adaptable.io. On another security note, I fear putting confidential info even in a private repo which is outsourced to a third party could be weakening a chain in the security pipeline. I can't have the contact information of everyone I'm sending my message to, let alone the content of the message saved as cleartext. It contains information on how to access my entire life which believe it or not I don't want the whole world knowing. I learned that you can store \\\"secret\\\" system environment variables, so I went with that for contact info and the message to send.</p><br><p class=\\\"blogtextlist\\\">- So I have achieved a free live 24/7 web server which can confidentially save the info I need, but how do I actually handle 1. knowing if I'm dead and 2. firing off the messages. To handle if I know that I'm dead, I figured the most realistic automatic \\\"health check\\\" would be whether or not my PC has turned on. So on my PC I set the system scheduler to run a .bat file posting the web server with a secret key to prove I'm still alive every hour my PC is on. If the web server hasn't received my ping in 3 months straight, then it fires off the messages technically only through email (but you can send emails to mobile numbers if you know the carrier) using Zoho.</p><br><p class=\\\"blogtextlist\\\">- Now one more challenge stood in my way, troubleshooting to prevent an early firing. The spookiest prospect of them all. I can't explain the due diligence I used to stress test the system exactly because there was a lot and it's probably boring, but the safety features I can share is I set the system to fire three warning messages only to me at different partial time intervals basically saying \\\"Hey you haven't pinged on your PC is something wrong or are you dead?!?!\\\". My last failsafe was I left a hard copy hidden deeply on google docs among other hundreds of documents shared with a trusted friend. The chain is only as strong as the weakest link and I'd say Adaptable.io getting breached and my secure environment variables leaked and/or a malicious actor within the company are threat one (though it would be obvious if I was still alive and they used this to access my bank and other stuff because this is threat one), and my Google getting accessed and the bad actor somehow managing to go through those hundreds of files to coincidentally come across the hidden file is threat two. I suppose someone accessing my PC to get the key stored in the .bat and infinitely keeping my deadman's switch pinged is a threat but what do they get out of that?<br><br>Edit: I originally used Replit as host but have since switched from to a GitHub/Adaptable.io CI/CD approach since Replit changed their pricing structure.</p><br>",
  fontfamily: "",
  fontsize: "",
  fontweight: "",
  color: "",
